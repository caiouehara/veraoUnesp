...
def descarte_de_transiente(porcentagem, serie_temporal):
    """Porcentagem: A pocentagem de pontos inciais da série que serão
        descartados (entre 0 e 1 :: entreja seja um número decimal)
    """
    """ Série_temporal: Resultado da integração numérica cuja parte inicial
        precisa ser descartada
    """
    # N representa o tamanho da série temporal
    N = len(serie_temporal)
    """Ponto inicial representa o primeiro ponto após o Python descartar
        a parte numérica da integração não tem utilidade
    """
    ponto_inicial = int(np.floor(porcentagem*N))
    serie_temporal = np.asarray(serie_temporal) #Convertendo a lista "serie_temporal" para array
    serie_recortada = serie_temporal[ponto_inicial::] #Série recortada = Serie_temporal (ponto inicial até o final)
